"""
Render samples and create gifs of the sampling process. Use the image_inference_msd.py script to only plot the final sample.
"""
...
from house_diffusion.modified_swiss_dwellings_housediffusion_dataset import load_modified_swiss_dwellings
...
def bin_to_int_sample(sample, resolution=512):
    ...
    for i in tqdm(range(sample.shape[1])):
        ...
        for k in range(sample.shape[0]):
            ...
            for poly, c in zip(polys, types):
                ...
                draw.append(drawsvg.Lines(*np.array(poly).flatten().tolist(), close=True, fill='black', fill_opacity=0.0, stroke=webcolors.rgb_to_hex([int(x/2) for x in c]), stroke_width=0.5*(resolution/512)))
                draw2.append(drawsvg.Lines(*np.array(poly).flatten().tolist(), close=True, fill=ID_COLOR[room_type], fill_opacity=1.0, stroke=webcolors.rgb_to_hex([int(x/2) for x in c]), stroke_width=0.5*(resolution/512)))
                for corner in poly:
                    draw.append(drawsvg.Circle(corner[0], corner[1], 2*(resolution/512), fill=ID_COLOR[room_type], fill_opacity=1.0, stroke='gray', stroke_width=0.25))
                    draw3.append(drawsvg.Circle(corner[0], corner[1], 2*(resolution/512), fill=ID_COLOR[room_type], fill_opacity=1.0, stroke='gray', stroke_width=0.25))
            for poly, c in zip(polys, types):
            ...
            if k==sample.shape[0]-1 or True:
                if save_edges:
                    ...
                    print(f"saved to: {f'outputs/{ext}/{tmp_count+i}_{k}_{ext}.svg'}")
                if save_svg:
                    draw_color.save_svg(f'outputs/{ext}/{tmp_count+i}c_{k}_{ext}.svg')
                    print(f"saved to: {f'outputs/{ext}/{tmp_count+i}c_{k}_{ext}.svg'}")
        ...
        if save_gif:
            imageio.mimwrite(f'outputs/gif/{tmp_count+i}.gif', images, duration=100, loop=1)
            imageio.mimwrite(f'outputs/gif/{tmp_count+i}_v2.gif', images2, duration=100, loop=1)
            imageio.mimwrite(f'outputs/gif/{tmp_count+i}_v3.gif', images3, duration=100, loop=1)
    return graph_errors
...
def main():
    ...
    logger.configure(log_suffix="_sample")
    ...
    os.makedirs('outputs/pred', exist_ok=False)
    os.makedirs('outputs/gt', exist_ok=False)
    os.makedirs('outputs/gif', exist_ok=False)
    os.makedirs('outputs/graphs_gt', exist_ok=False)
    os.makedirs('outputs/graphs_pred', exist_ok=False)

    errors = []
    for sample_iteration in range(1):
        logger.log("sampling...")
        tmp_count = 0
        ...
        elif args.dataset=='modified_swiss_dwellings':
            ID_COLOR = {0: '#660101', 1: '#EE4D4D', 2: '#C67C7B', 3: '#FFD274', 4: '#BEBEBE', 5: '#BFE3E8',
                        6: '#7BA779', 7: '#E87A90', 8: '#FF8C69', 10: '#1F849B', 11: '#727171',
                        13: '#785A67', 12: '#D3A2C7'}
            num_room_types = 12
        ...
        while tmp_count < args.num_samples:
            ...
            os.makedirs('output_debugs', exist_ok=True)
            with open("output_debugs/sample.pkl", "wb") as f:
                th.save(sample, f)
            
            with open("output_debugs/sample_gt.pkl", "wb") as f:
                th.save(sample_gt, f)

            graph_error = save_samples(sample_gt, 'gt', model_kwargs, tmp_count, num_room_types, ID_COLOR=ID_COLOR, draw_graph=args.draw_graph, save_svg=args.save_svg, save_gif=args.save_gif)
            graph_error = save_samples(sample, 'pred', model_kwargs, tmp_count, num_room_types, ID_COLOR=ID_COLOR, is_syn=True, draw_graph=args.draw_graph, save_svg=args.save_svg, save_gif=args.save_gif)
        ...
        try:
            ...
            logger.logkv("compatibility", np.mean(graph_errors))

        except:
            print("FID calculation failed")

        logger.log_image_folder("outputs/pred/", glob_str="*.png")
        logger.log_image_folder("outputs/gt/", glob_str="*.png")

        logger.log_video_folder("outputs/gif/", glob_str="*.gif")


        logger.log_save_dir("outputs/pred/*.png", "outputs", "now")
        logger.log_save_dir("outputs/gt/*.png", "outputs", "now")
        logger.log_save_dir("outputs/gif/*.gif", "outputs", "now")


        # Move save_path
        save_dir_path = f"model_outputs/{args.model_path.replace('/', '-')}/{args.set_name}-{args.target_set}/{sample_iteration}/"

        try:
            os.makedirs(save_dir_path, exist_ok=True)
            os.system(f"cp -r outputs/* {save_dir_path}")
        except:
            pass
