...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        if t[0] < predict_descrete:
            ...
            model_output_bin = bin2dec(model_output_bin.round().int().permute([0,2,1]).reshape(model_output_bin.shape[0],
                model_output_bin.shape[2], 2, self.num_bits), self.num_bits).permute([0,2,1])

            model_output_bin = ((model_output_bin/self.img_size) - 0.5) * 2
    ...
    def p_sample_loop(
        ...
        for i, sample in tqdm(enumerate(self.p_sample_loop_progressive(
            ...
            if i % return_every_nth == 0:
                samples.append(sample['sample'])
                timesteps.append(sample["t"])
    ...
    def training_losses(self, model: TransformerModel | th.nn.Module, x_start, t, model_kwargs, analog_bit, noise=None):
        """
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            if not analog_bit:
                ...
                bin_target = bin_target * self.img_size #-> [0, 256]
                
                bin_target = dec2bin(bin_target.permute([0,2,1]).round().int(), self.num_bits) 
                bin_target = bin_target.reshape([target.shape[0], target.shape[2], 2 * self.num_bits]).permute([0,2,1])
                ...
                assert model_output_bin.shape == bin_target.shape, f"{model_output_bin.shape} != {bin_target.shape}"
